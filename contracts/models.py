...
class Provenance(BaseModel):
    ...
    input_sha: str = Field(
        description="本行输入 (item_key 或拼接文本) 的 sha256[:16]，重放核对"
    )
...
class RawSource(BaseModel):
    ...
    item_guid: str | None = Field(default=None, description="源站原生 guid/id")
...
class RawFetch(BaseModel):
    ...
    reachable: bool = True
    etag: str | None = None
    content_sha256: str | None = Field(default=None, description="原始响应体 hash[:16]")
...
class RawItem(BaseModel):
    ...
    url_canon: str = Field(
        description="规范化 URL：去 utm_*/fbclid/www./尾斜杠，https 归一"
    )
    title: str
    content_text: str | None = Field(
        default=None, description="feed summary/正文纯文本"
    )
    ...
    date_published: str | None = Field(default=None, description="源站发布时间，可为空")
    date_fetched: str = RFC3339
    language: str | None = Field(default=None, description="BCP-47-ish: zh/en/…")
...
class FilterVerdict(BaseModel):
    model_config = ConfigDict(extra="forbid")
    schema_: Literal["filter_verdict/1"] = Field(
        default="filter_verdict/1", alias="schema"
    )
    item_key: str = ItemKey
    ...
    news_value: float | None = Field(default=None, ge=0, le=1)
...
class DedupVerdict(BaseModel):
    model_config = ConfigDict(extra="forbid")
    schema_: Literal["dedup_verdict/1"] = Field(
        default="dedup_verdict/1", alias="schema"
    )
    item_key: str = ItemKey
    verdict: Literal["fresh", "suppressed", "reissue", "gray"]
    cluster_id: int | None = Field(
        default=None, description="history.db clusters.cluster_id"
    )
    match_cos: float | None = Field(default=None, ge=-1, le=1)
    judge: dict | None = Field(default=None, description="LLM judge 判词原文")
...
class KeptItem(BaseModel):
    ...
    note: str | None = Field(default=None, description="人工批注（可空）")
...
class AudioManifest(BaseModel):
    model_config = ConfigDict(extra="forbid")
    schema_: Literal["audio_manifest/1"] = Field(
        default="audio_manifest/1", alias="schema"
    )
    episode: str = Episode
...
class TimelineSeg(BaseModel):
    ...
    words: list[TimelineWord] | None = Field(
        default=None,
...
class TimelineOverlay(BaseModel):
    ...
    at_sentences: list[int] = Field(
        description="来源句区间（编译前的语义输入，审计用）"
    )
...
class Timeline(BaseModel):
    ...
    gap: dict = Field(
        default_factory=lambda: {"sentence": 0.22, "item": 0.55},
...
class FramesManifest(BaseModel):
    model_config = ConfigDict(extra="forbid")
    schema_: Literal["frames_manifest/1"] = Field(
        default="frames_manifest/1", alias="schema"
    )
    episode: str = Episode
    dir: str
...
class BuildManifest(BaseModel):
    ...
    inputs: dict = Field(
        description="{render_plan,timeline,audio_manifest,frames_manifest}: sha256:…"
    )
    tool: BuildTool
...
class StageEntry(BaseModel):
    ...
    produced_at: str | None = RFC3339
    producer: str | None = Field(default=None, description="脚本/人/模型标识")
...
class RunMeta(BaseModel):
    ...
    stages: dict[str, StageEntry] = Field(
        description="stage 名 → 产物登记；DAG 断点续跑的依据"
    )
