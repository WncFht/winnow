...
- **Stages run inside one uv project.** The root `pyproject.toml` declares all Python deps and installs `stages`/`adapters`/`contracts` as an editable package (`uv sync` → `.venv/`). Run stages via `uv run stages/<stage>.py --run-dir runs/<date>`; imports are fully qualified (`from stages.lib import …`, `from contracts import …`) — no `sys.path` wiring. `stages/lib/*.py` are shared modules; most have a `__main__` self-check (`--selftest`, or `--offline` to skip live-network asserts). `tools/tts_workers/` is a separate heavy subproject (torch/transformers) that keeps its own PEP 723 deps.
