...
`.gitdir/` 在 .gitignore 中、不随仓分发——fresh clone 里没有它，先重建（克隆上游 → 把它的 `.git` 挪成 `.gitdir`，工作树壳删掉）：
...
备选（不恢复 gitdir）：scratch 目录 clone 上游并 checkout 定格 SHA， `diff -r` 对比 vendored 树即可（排除 node_modules/ 与上方本地 patch 清单）。
