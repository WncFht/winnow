...
import { writeHtmlForFileRender } from '../src/utils/vendor-assets';
...
export async function renderPng(body: Json, authHeader: string | null): Promise<Buffer> {
  ...
  try {
    ...
    // CDN 自托管后 HTML 使用相对 vendor/… 引用：about:blank 的 setContent
    // 无法解析，改为写盘 + <base> 注入 + file:// 导航。
    const { htmlPath, fileUrl } = writeHtmlForFileRender(
      html,
      path.join(process.cwd(), 'output', '.render-cache'),
      `ssr-${process.pid}-${Date.now()}.html`,
    );
    await page.goto(fileUrl, { waitUntil: 'load', timeout: 12_000 });
    try {
